...
class Vehicle:
    ...
    def plot(self, x, y, yaw, steer=0.0, cabcolor="-r", truckcolor="-k"):
        outline = np.array([
            [-self.BACKTOWHEEL, (self.LENGTH - self.BACKTOWHEEL), (self.LENGTH - self.BACKTOWHEEL), -self.BACKTOWHEEL,
             -self.BACKTOWHEEL],
            [self.WIDTH / 2, self.WIDTH / 2, - self.WIDTH / 2, -self.WIDTH / 2, self.WIDTH / 2]
        ])
        fr_wheel = np.array([[self.WHEEL_LEN,
                              -self.WHEEL_LEN,
                              -self.WHEEL_LEN,
                              self.WHEEL_LEN,
                              self.WHEEL_LEN],
        ...
        if self.patches:
            ...
            p_outline, = plt.plot(outline[0, :], outline[1, :], truckcolor, linewidth=0.5)
            p_fr_wheel, = plt.plot(fr_wheel[0, :], fr_wheel[1, :], truckcolor, linewidth=0.5)
            p_rr_wheel, = plt.plot(rr_wheel[0, :], rr_wheel[1, :], truckcolor, linewidth=0.5)
            p_fl_wheel, = plt.plot(fl_wheel[0, :], fl_wheel[1, :], truckcolor, linewidth=0.5)
            p_rl_wheel, = plt.plot(rl_wheel[0, :], rl_wheel[1, :], truckcolor, linewidth=0.5)
            p_center, = plt.plot(x, y, "+")

            self.patches = [p_outline, p_fr_wheel, p_rr_wheel, p_fl_wheel, p_rl_wheel, p_center]
...
if __name__ == "__main__":
    ...
    ani = animation.FuncAnimation(fig, update, frames=np.arange(0, 100), init_func=init, blit=False, repeat=False)
